load("result_process/sensitivity_1_us.RData")
...
for (i in intervention){
  b <- grep(ls(), pattern = paste0("us_", i) , value = TRUE)
  effect <- c()
  for (j in b){
    if (is.list(get(j))){
...
r = 0
for (i in 1:length(a)){
  if(names(a[i]) %in% c("stay_at_home", "school_close", "childcare_close",
                        'shop_close', 'restaurant_close', 'bar_close', "entertainment_close",
                        "cultural_close", "worship_close", "sports_indoor_close", "sports_outdoor_close",
                        "gathering_outside_10lower", "gathering_outside_10over")){
    r = r + 1
...
write.csv(result,"plot/fig_s5/sensitivity_1_us.csv")
