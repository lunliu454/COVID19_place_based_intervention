load("result_process/effect_two_stages.RData")
...
a <- sapply(c(0:20), function(x){paste0("day_", x , '=0,day_', x , '_se=0,day_', x ,"_size=0")}) %>%
  paste(collapse = ",")
a <- paste0("data.frame(intervention=rep(intervention,2),phase=c(rep('down',16),rep('lift',16)),effect_average=0,se_effect_average=0,",
              a,",stringsAsFactors=F)")
result <- eval(parse(text=a))
b <- grep(ls(), pattern = "bra_", value = TRUE)
b <- grep(b, pattern = "down", value=TRUE)
c <- grep(ls(), pattern = "bra_", value=TRUE)
