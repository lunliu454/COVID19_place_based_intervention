load("result_process/sensitivity_1_bra.RData")
...
for (i in intervention){
  b <- grep(ls(), pattern = paste0("bra_", i) , value = TRUE)
  effect <- c()
  for (j in b){
    if (is.list(get(j))){
...
result <- as.data.frame(matrix(nrow = 13, ncol = length(a[[2]]) + 1))
colnames(result) <- c("intervention", 1:length(a[[1]]))
r = 0
for (i in 1:length(a)){
  if(names(a[i]) %in% c("stay_at_home", "school_close",
                        "office_close", 'shop_close',
                        'restaurant_close', 'bar_close',
                        "entertainment_close", "cultural_close",
                        "worship_close", "sports_indoor_close",
                        "sports_outdoor_close", "gathering_outside_10lower",
...
result$intervention <- c("Stay-at-home", "School+childcare closure", "Office closure", 
...
write.csv(result,"plot/fig_s5/sensitivity_1_bra.csv")
