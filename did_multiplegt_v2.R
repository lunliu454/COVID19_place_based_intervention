...
generate_num_names <- function(dynamic = 0) {
  names = c("N_effect")
  if (dynamic > 0) {
    for (i in 1:dynamic) {
      nm_N_num_dynamic = paste("N", "dynamic", toString(i), sep = "_")
      names = c(names, nm_N_num_dynamic)
    }
  }
  names
} #
...
did_multiplegt_transform <- function(df, controls, trends_nonparam, controlby, period) {
  ...
  if (controlby=="period"){
    i<-1
    df <- df %>% mutate(periodG=ifelse(.data$T<=period[1],1,NA))
    if (length(period)>1){
      i<-i+1
      while(i<=length(period)){
        df <- df %>% mutate(periodG=ifelse(
          .data$T>period[i-1]&.data$T<=period[i],i,periodG))
        i<-i+1
      }
      df <- df %>% mutate(periodG=ifelse(.data$T>period[i-1],i,periodG))
    }
  }
  ...
  # df <- df %>%
  #   ungroup() %>%
  #   mutate(tag = rowSums(is.na(.[c(controls_rename,"D")]))) %>%
  #   mutate_at(.vars = vars(-ignore_columns), .funs = list(~ ifelse(.data$tag, NA, .))) %>% select(-.data$tag)
...
dynamic_transform_level <- function(df, counter_dynamic, controls) {
  ...
  df_l <- df %>%
    group_by(.data$G) %>%
    select_at(.vars = vars(c("T", "G", "Y", "D", nm_ctrls))) %>%
    mutate_at(.vars = vars("Y"), .funs = function(x) { x - lag(x, counter_dynamic+1) }) %>% #
    mutate_at(.vars = vars(nm_ctrls), .funs = function(x) {#
      lag_sum <- 0#
      for (counter_dynamic_i in 1:(counter_dynamic+1)){#
        lag_sum <- lag_sum+lag(x, counter_dynamic_i)#
      }#
      x - lag_sum/(counter_dynamic+1) #
    }) %>%#
    mutate_at(.vars = vars("D"), .funs = function(x) {#just for check equal to x - lag(x, counter_dynamic+1)
      lag_sum <- 0#
      for (counter_dynamic_i in 1:counter_dynamic){#
        lag_sum <- lag_sum+lag(x, counter_dynamic_i)#
      }#
      x - lag_sum/counter_dynamic #
    }) %>%#
    rename_at(.vars = vars(c("Y", nm_ctrls)), .funs = function(x) { fn_diff_i_rename(x, counter_dynamic) }) %>%
    rename_at(.vars = vars("D"),.funs = function(x){"diff_D"}) %>%
                                      ...
                                      cluster, controlby, period)
{
  ...
  df <- did_multiplegt_transform(df, controls, !is.null(trends_nonparam), controlby, period)
...
prepare_placebo_once <- function(df, counter_placebo, controls, threshold_stable_treatment) {
  ...
  df <- df %>%
    ...
    rename_at(.vars = vars(c(nm_lag_diff_Y, nm_lag_diff_controls)), .funs = list(~ stringr::str_remove(., prefix))) %>%
...
prepare_dynamic_once <- function(df, counter_dynamic, controls, threshold_stable_treatment) {
  ...
  df <- df %>%
    ...
    rename_at(.vars = vars(c(nm_lead_diff_i_Y, nm_lead_diff_i_controls)), .funs = list(~ stringr::str_replace(., prefix, "diff_"))) %>%
...
residuals_control_once <- function(df, d, threshold_stable_treatment, diff_ctrl_names, trends_nonparam, trends_lin, 
                                   controlby, periodg, controlT) {
  if (controlby=="Dlevel"){
    df_d <- df %>% filter( .data$DgroupLag == d )
  }
  if (controlby=="period"){
    df_d <- df %>% filter( .data$periodG == periodg )
  }
  df_d <- as.data.frame(df_d)
  df_stable <- df_d %>% filter( abs(.data$diff_D) <= threshold_stable_treatment ) %>% filter(!is.na(.data$diff_Y))
  df_stable <- as.data.frame(df_stable)
  n_stable <- nrow(df_stable)
  do_count_tfactor <- (length(unique(df_stable$Tfactor)) > 1) & controlT
  rm_ctrl <- c()
  for (ctrl in diff_ctrl_names){ #
    a <- df_stable[ctrl][!is.na(df_stable[ctrl])] #
    b <- df_d[ctrl][!is.na(df_d[ctrl])] #
    if(length(a)>0 & length(b)>0){ #
      if(max(a)!=0|min(a)!=0){ #
        if((max(a)-min(a))/(max(b)-min(b))<=0.5001){ #
          rm_ctrl <- c(rm_ctrl,ctrl) #
        }
      }
    }
  } #
  diff_ctrl_names <- setdiff(diff_ctrl_names,rm_ctrl) #
  ...
  if (NA %in% df_stable.lm$coefficients){
    diff_ctrl_names <- diff_ctrl_names[which(!is.na(df_stable.lm$coefficients))-1]
    if (length(diff_ctrl_names)>0){
      formula = paste(diff_ctrl_names, collapse = " + ")
      formula = paste("diff_Y ~ ", formula, sep = "")
      df_stable.lm <- lm(formula, data = df_stable, weights = counter)
  ...
  if (length(diff_ctrl_names)>1){
    vif_test <- car::vif(df_stable.lm)
  }else{
    vif_test <- NA
  }
  while(TRUE %in% unlist(vif_test>10)){
    diff_ctrl_names <- diff_ctrl_names[-which.max(vif_test)]
    formula = paste(diff_ctrl_names, collapse = " + ")
    formula = paste("diff_Y ~ ", formula, sep = "")
    df_stable.lm <- lm(formula, data = df_stable, weights = counter)
    if(length(vif_test)>2){
      vif_test <- car::vif(df_stable.lm)
    }else{break}
  }
...
residuals_control <- function(df, controls, threshold_stable_treatment, trends_nonparam, trends_lin, controlby, period, controlT) {
  ...
  if (controlby=="Dlevel"){
    ...
    for (d in d_min:d_max) {
      tmp_ret <- residuals_control_once(
        df, d, threshold_stable_treatment, diff_ctrl_names, trends_nonparam, trends_lin, controlby, periodg, controlT)
      df <- df %>%
  ...
  if (controlby=="period"){
    d = 0
    for (periodg in 1:(length(period)+1)) {
      tmp_ret <- residuals_control_once(
        df, d, threshold_stable_treatment, diff_ctrl_names, trends_nonparam, trends_lin, controlby, periodg, controlT)
      df <- df %>%
        left_join(tmp_ret, by = c("T", "G")) %>%
        mutate(diff_Y = ifelse(.data$periodG == periodg, residuals, .data$diff_Y)) %>%
        select(-residuals)
    }
  }
  df
...
did_multiplegt_once <- function(df, d, threshold_stable_treatment,direction) {
...
did_multiplegt_core <- function(df, controls, threshold_stable_treatment, trends_nonparam, trends_lin,
                                counter_placebo = 0, counter_dynamic = 0, 
                                t_max = 0, direction = direction, 
                                controlby = controlby, period = period, 
                                controlT = controlT) {
  effect = 0
  denom = 0
  total = 0
  switchers = 0
  df <- residuals_control(df, controls, threshold_stable_treatment, trends_nonparam, trends_lin, controlby, period, controlT)
  for (t in (counter_placebo + 2):(t_max - counter_dynamic)) {
    ...
    if (nrow(df_d) > 0) {
      ...
      for (d in d_min:d_max) {
        tmp_ret = did_multiplegt_once(df_d, d, threshold_stable_treatment, direction)
...
did_multiplegt_estim <- function(df, controls, placebo, dynamic, threshold_stable_treatment, trends_nonparam, trends_lin,
                                  direction, controlby, period, controlT) {
  t_max <- max(df$T)
  ...
  if (dynamic > 0) {
    ...
    for (i in 1:dynamic) {
      ...
      if(is.nan(ret_dynamic_lag$effect)){break}
    }
  }
...
did_multiplegt_bootstrap = function(df, controls, brep, trends_nonparam, trends_lin, has_cluster = FALSE,
                                    ...
                                    parallel = FALSE, direction = "both", controlby, period, controlT
                                    ) {
  ...
  callback = function(i) {
    samples = if (has_cluster) {
      ...
      sampling::getdata(df, cs1)
    } else {
      sampling::getdata(df, srswr(N, N))
    }
    
    ret = c()
    rtmp = did_multiplegt_estim(samples, controls, placebo, dynamic, threshold_stable_treatment, trends_nonparam, trends_lin, direction,
                                controlby, period, controlT)
    for (name in names) {
  ...
  if (parallel) {
    ...
    matrix(unlist(mclapply(1:brep, callback, mc.cores = numCores)), nrow = brep, ncol = K, byrow = TRUE) #mac
    } else { 
...
df=input
Y="log_win7_Rt_estimate"
G="city_eng_name"
T="date"
D="win7_gathering_outside_10over"
controls = control[control!=as.character(p)]
cluster="date"
placebo = 0
dynamic = 2
threshold_stable_treatment = 0.01
...
direction = "both"
controlby = "period"
period = c("2020-03-28","2020-06-13")
controlT = FALSE

did <- function(df, Y, G, T, D, controls = c(),
                placebo = 0, dynamic = 0,
                ...
                controlT = TRUE)
{
  ...
  eret = did_multiplegt_estim(df_preprocess, controls, placebo, dynamic, threshold_stable_treatment,
                              trends_nonparam, trends_lin, direction, controlby, period, controlT)
  if (brep == 0) {
    eret
  } else {
    dynamic = (length(eret)-placebo*2)/3-1
    ...
    num_names = generate_num_names(dynamic)
    ret = NULL
    X = c()
    LB = c()
    UB = c()
    for (i in 1:length(names)) {
      ...
      s = as.numeric(sd(bret[,i],na.rm=TRUE))
      N = eret[Nname]
    ...
    if (covariance) {
      ...
      for (i in 0:dynamic) {
        ...
        for (j in (i+1):dynamic) {
          r = cov(bret[,i+base], bret[,j+base],use="na.or.complete" )
      ...
      if (!is.null(average_effect)) {
        ...
        if (dynamic >= 0){
          ...
          } else if (average_effect == "prop_number_min") { #
            a = unlist(eret[switcher_names[start:end]])#
            b = unlist(eret[num_names])#
            weight = mapply(min,b-a,a) #
            sum_weight = sum(weight) #
            weight = weight / sum_weight #
          } #
          
          effect_average = sum(unlist(eret[names[start:end]]) * weight,na.rm=TRUE)
          ...
          ret["effect_average"] = NA
          ret["N_effect_average"] = NA
          ret["se_effect_average"] = NA
        }
      }
    }
