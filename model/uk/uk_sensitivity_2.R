...
periods <- list(c("2020-03-30","2020-06-04"),
                c("2020-03-29","2020-06-08"),
                c("2020-03-28","2020-06-18"),
                c("2020-03-27","2020-07-05"),
                c("2020-03-27","2020-07-07"))
cut_off_value <- c("09", "08", "07", "06", "05")
for (i in 1:5){
  for (p in control[2:17]){
    model <- p %>% str_sub(6, -1)
    model <- paste("uk", cut_off_value[i], model, sep = "_")
    m <- try(did(input, "log_win7_Rt_estimate", "city_eng_name", "date", p,
                 ...
                 controlT = FALSE))
    assign(model,m)
    save.image("sensitivity_2_uk.RData")
  }
}
