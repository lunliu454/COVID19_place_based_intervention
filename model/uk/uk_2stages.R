...
for (p in control[2:17]){
  #intervention effects in the locking down stage
  model <- p %>% str_sub(6, -1)
  model <- paste0("uk_down_", model)
  m <- try(
    did(input, "log_win7_Rt_estimate", "city_eng_name", "date", p,
        ...
        parallel = TRUE, 
        direction = "increase", 
        controlby = "period", period=c("2020-03-29","2020-06-08")),
        controlT = FALSE)
  assign(model,m)
  
  #intervention effects in the reopening stage
  model <- p %>% str_sub(6, -1)
  model <- paste0("uk_lift_", model)
  m <- try(
    did(input, "log_win7_Rt_estimate", "city_eng_name", "date", p,
        controls = control[control != p],
        placebo = 0, dynamic = 20, cluster = "date",
