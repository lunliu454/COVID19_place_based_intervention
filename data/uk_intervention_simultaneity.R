input <- read.csv("data/uk_input.csv",stringsAsFactors = F)
input <- input %>% 
  mutate(date = as.Date(date)) %>% 
  filter(date >= as.Date("2020-03-21")) #when started to have non-NA rt
...
for (j in c("down", "lift")){
  ...
  for (i in 1:length(cities)){
    ...
    b$city <- cities[i]
    colnames(b)[1:16] <- colnames(tmp)[2:17]
    tmp <- rbind(tmp, b)
  }
  tmp <- tmp[-1, ]
  #identify interventions that didn't change in each stage
  unchanged <- apply(tmp[, 2:17], 2, function(x){F %in% is.na(x)})
...
write.csv(table_2_down,"corr_uk_down.csv")
write.csv(table_2_lift,"corr_uk_lift.csv")
