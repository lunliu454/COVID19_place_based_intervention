input <- read.csv("brazil_input_git.csv",stringsAsFactors = F)
input <- input %>% 
  mutate(date = as.Date(date)) %>% 
  filter(date >= as.Date("2020-01-27")) #when started to have non-NA rt
...
for (j in c("down", "lift")){
  ...
  for (i in 1:length(cities)){
    ...
    b$city <- cities[i]
    colnames(b)[1:14] <- colnames(tmp)[2:15]
    tmp <- rbind(tmp, b)
  }
  tmp <- tmp[-1, ]
  #identify interventions that didn't change in each stage
  unchanged <- apply(tmp[, 2:15], 2, function(x){F %in% is.na(x)})
...
for (k in c("down", "lift")){
  ...
  tmp <- as.data.frame(matrix(nrow = 14, ncol = 14), row.names = intervention)
...
write.csv(table_2_down,"corr_kor_down.csv")
write.csv(table_2_lift,"corr_kor_lift.csv")
