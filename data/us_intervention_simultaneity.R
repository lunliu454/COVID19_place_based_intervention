...
for (j in c("down", "lift")){
  ...
  for (i in 1:length(cities)){
    ...
    b$city <- cities[i]
    colnames(b)[1:13] <- colnames(tmp)[2:14]
    tmp <- rbind(tmp, b)
  }
  tmp <- tmp[-1, ]
  #identify interventions that didn't change in each stage
  unchanged <- apply(tmp[, 2:14], 2, function(x){F %in% is.na(x)})
...
write.csv(table_2_down,"corr_us_down.csv")
write.csv(table_2_lift,"corr_us_lift.csv")
